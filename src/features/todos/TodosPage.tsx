  ...
  return (
    <div className="h-full p-8 flex flex-col select-none">
      ...
      <div className="flex gap-2 mb-8">
        ...
        />
        <ThemedButton onClick={handleAdd} className="h-14 w-14 p-0 text-xl">
          <Plus size={28} />
        </ThemedButton>
      </div>
      <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
        ...
        {todos.map(todo => (
          <div key={todo.id} className="flex items-center justify-between border-b border-current/10 pb-4 group">
            <div
              className="flex items-center gap-6 flex-1 py-2 cursor-pointer"
              onClick={() => toggleTodo(todo.id, todo.text, !!todo.done)}
            >
              <Checkbox
                checked={!!todo.done}
                className="h-8 w-8 border-current rounded-none data-[state=checked]:bg-current data-[state=checked]:text-background"
              />
